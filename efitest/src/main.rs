...
use custom_processing_unit::{
    apply_hook_patch_func, apply_patch, disable_all_hooks, hook, labels, ms_hook_read,
    ms_hook_write, ms_patch_instruction_read, ms_patch_instruction_write, ms_seqw_read,
    ms_seqw_write, read_patch, stgbuf_read, stgbuf_write_raw, CustomProcessingUnit,
};
use data_types::addresses::{MSRAMHookIndex, MSRAMSequenceWordAddress, UCInstructionAddress};
use log::info;
...
fn ldat_read() {
    ...
    assert_eq!(
        ...
        rdrand_patch_read_before.len()
    );
    read_patch(
        ldat_read.addr,
        rdrand_patch.addr,
        &mut rdrand_patch_read_before,
    );
    ...
    for (i, ((should_be, read_value), before_value)) in rdrand_patch
        .ucode_patch
        .iter()
        .zip(rdrand_patch_read.iter())
        .zip(rdrand_patch_read_before.iter())
        .enumerate()
    {
        for offset in 0..3 {
            ...
            println!(
                "[{}] {:013x} {:013x} {:013x} {}",
                address, should_be[offset], read_value[offset], before_value[offset], different
            );
        }
    }
    ...
    for i in 0..128 * 3 {
        ms_patch_instruction_write(UCInstructionAddress::MSRAM_START.patch_offset(i), i);
    }
    ...
    for i in 0..128 * 3 {
        ...
        print!("[{i:04x}] ");
        let val = ms_patch_instruction_read(
            ldat_read.addr,
            UCInstructionAddress::MSRAM_START.patch_offset(i),
        );
        let difference = if val != i {
            mismatch = true;
            "<"
        } else {
            ""
    ...
    for i in 0..128 {
        ms_seqw_write(MSRAMSequenceWordAddress::ZERO + i, i);
    }
    apply_patch(&ldat_read);
    for i in 0..128 {
        print!("[{i:04x}] ");
        let val = ms_seqw_read(ldat_read.addr, MSRAMSequenceWordAddress::ZERO + i);
        let difference = if val != i {
            mismatch = true;
            "<"
        } else {
    ...
    for i in 0..31 {
        // only till 31, since hook with index 31 alread occupied
        ms_hook_write(MSRAMHookIndex::ZERO + i, i);
    }
    apply_patch(&ldat_read);
    for i in 0..32 {
        print!("[{i:04x}] ");
        let val = ms_hook_read(ldat_read.addr, MSRAMHookIndex::ZERO + i);
        let difference = if val != i {
            mismatch = true;
            "<"
        } else {
