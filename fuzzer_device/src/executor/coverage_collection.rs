...
use core::pin::Pin;
use coverage::harness::coverage_harness::{CoverageError, CoverageExecutionResult, CoverageHarness};
...
use coverage::harness::iteration_harness::{IterationHarness, IterationRun};
...
impl CoverageCollector {
    pub fn initialize() -> Result<Self, custom_processing_unit::Error> {
        ...
        let interface = match ComInterface::new(&interface_definition::COM_INTERFACE_DESCRIPTION) {
            ...
            Err(e) => {
                return Err(custom_processing_unit::Error::Other(format!("Failed to create COM interface: {:?}", e)));
            }
        };
        ...
        if hooks == 0 {
            return Err(custom_processing_unit::Error::Other("No hooks available".to_string()));
        }

        let interface_static: &'static mut ComInterface = unsafe { core::mem::transmute(&mut interface) };
        ...
        let harness = match CoverageHarness::new(interface_static, cpu_static) {
            Ok(harness) => harness,
            Err(e) => {
                return Err(custom_processing_unit::Error::Other(format!("Failed to initiate harness {:?}", e)));
            }
        };
    ...
    pub fn execute_once<FuncResult, F: FnOnce() -> FuncResult>(
        &mut self,
    ...
    pub fn execute_all<FuncResult, F: FnMut(&[UCInstructionAddress]) -> FuncResult>(
        &self,
        func: F,
    ) -> IterationRun<FuncResult, F> {
        self.iteration_harness.execute(func)
    }
}
