...
use crate::executor::coverage_collection::CustomProcessingUnitData;
...
pub struct SampleExecutor {
    ...
    custom_processing_unit: Option<CustomProcessingUnitData>,
}
...
impl SampleExecutor {
    pub fn execute_sample(&mut self, sample: &Sample, execution_result: &mut ExecutionResult) {
        ...
        self.prepare_vm_state(&sample.code_blob);
    ...
    pub fn new() -> Result<Self, HypervisorError> {
        let mut custom_processing_unit = CustomProcessingUnit::new().map(Option::Some).unwrap_or_else(|error| {
            warn!("Failed to initialize custom processing unit: {:?}", error);
            warn!("Coverage collection will be disabled");
            None
        });

        if let Some(mut cpu) = custom_processing_unit {
            if let Err(err) = cpu.init() {
                error!("Failed to initialize custom processing unit: {:?}", err);
                return Err(HypervisorError::FailedToInitializeHost("custom processing unit"));
            }
        ...
        let mut code_page = Page::alloc_zeroed();
        let mut stack_page = Page::alloc_zeroed();
        let mut gdt_page = Page::alloc_zeroed();
        let mut tss_page = Page::alloc_zeroed();
        ...
        // build page table
        let mut page_table_4: *mut PML4Entry = page_table_4_page.as_mut_ptr().cast();
        unsafe {
            for i in 0..512 {
        ...
        let mut page_table_3: *mut PDPTEntry = page_table_3_page.as_mut_ptr().cast();
        unsafe {
            for i in 0..512 {
    ...
    fn prepare_vm_state(&mut self, code: &[u8]) {
    ...
    pub fn selfcheck(&mut self) -> bool {
        ...
        if let VmExitReason::Exception(ExceptionQualification {
            ...
            self.vm.vt.save_state(&mut self.initial_state);
