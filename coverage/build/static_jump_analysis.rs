...
fn analyse_jumps(rom: &RomDump<'static, 'static>) {
    ...
    let mut jump_towards: BTreeMap<UCInstructionAddress, Vec<UCInstructionAddress>> =
        BTreeMap::new();

    for (i, sequence_word) in rom.sequence_words().iter().enumerate() {
        let sequence_word = SequenceWord::disassemble_no_crc_check(*sequence_word)
            .expect("Failed to disassemble sequence word");

        if let Some(goto) = sequence_word.goto() {
            jump_towards.entry(goto.value).or_default().push(
                UCInstructionAddress::from_const(i * 4).with_triad_offset(goto.apply_to_index),
            );
        }
    }
    ...
    println!(
        "Total odd sequence word jump targets: {}",
        jump_towards.iter().filter(|x| x.0.is_odd()).count()
    );
    ...
    let dissassembly = ROMS_DISASM
        .iter()
        .find(|x| x.model() == rom.model())
        .unwrap()
        .dissassembly();
    for line in dissassembly.lines() {
        if let Some((address, content)) = line.split_once(": ") {
            ...
            let content_no_goto = regex_find_goto
                .replace_all(content.trim(), "GOTO X$1")
                .to_string();
            ...
            for capture in regex_find_address.captures_iter(&content_no_goto) {
                ...
                jump_towards
                    ...
                    .or_default()
                    .push(UCInstructionAddress::from_const(address));
            }
        }
    }
    ...
    println!(
        "Event targets: {}",
        jump_towards.iter().filter(|x| x.0.is_even()).count()
    );
    ...
    blacklist.push_str(
        std::fs::read_to_string("src/blacklist.txt")
            .expect("unable to read blacklist")
            .as_str(),
    );
    blacklist.push_str("\n\n");
    ...
    for x in &odd_targets {
        ...
        blacklist.push_str(&format!(
            "0x{:04x}, // excluded because maybe odd const jump from {:04x?} towards {}\n",
            target_base.address(),
            from,
            target
        ));
