...
use fuzzer_data::genetic_pool::{GeneticPool, GeneticPoolSettings};
...
pub async fn main(
        ...
        .await
    {
        ...
        if let Some(address) = net_receive_excluded_addresses(net).await {
            info!("Device excluded address last run: {}", address);
            database.blacklisted_addresses.insert(address);
        }
    }

    for blacklist in &database.blacklisted_addresses.iter().chunks(1000) {
        if let Err(err) = net
