...
impl SequenceWord {
    ...
    pub fn assemble(&self) -> u32 {
        let (tetrad_uidx, tetrad_address) = self.goto.clone().map(
            |value| (value.apply_to_index.into(), (value.value.address() & 0x7fff) as u32)
        ).unwrap_or((0x3u32, 0x0u32));

        let (sync_uidx, sync_ctrl) = self.sync.clone().map(
            |value| (value.apply_to_index.into(), value.value as u32)
        ).unwrap_or((0x3u32, 0x0u32));

        let (uop_uidx, uop_ctrl) = self.control.clone().map(
            |value| (value.apply_to_index.into(), value.value as u32)
        ).unwrap_or((0x0u32, 0x0u32));

        let seqw = (sync_ctrl << 25) | (sync_uidx << 23) | ( (tetrad_address & 0x7fff) << 8) | (tetrad_uidx << 6) | (uop_ctrl << 2) | uop_uidx;
...
mod test {
    ...
    use crate::utils::SequenceWordSync::{LFNCEMARK, SYNCFULL};
    ...
    fn check(label: [&str; 3], seqw: u32) {
        ...
        let lines = output.stdout.lines().map(|line| line.unwrap()).collect::<Vec<String>>();

        let regex = regex::Regex::new(r"\s*\[uop[0-2]]\s*").unwrap();

        let uops = lines.iter().skip(4).take(3).map(
            |s| regex.replace_all(s, "").trim().to_string()).collect::<Vec<String>>();
    ...
    fn test_sequencewords() {
        ...
        check(["", "SEQW GOTO U7c00", ""], SequenceWord::new().goto(1, UCInstructionAddress::MSRAM_START).assemble());
        check(["LFNCEMARK->", "SEQW UEND0", "SEQW GOTO U7c00"], SequenceWord::new().goto(2, UCInstructionAddress::MSRAM_START).sync(0, LFNCEMARK).control(1, UEND0).assemble());
    }
}
