...
impl hardware_vt::HardwareVt for Vmx {
    ...
    fn load_state(&mut self, state: &VmState) {
        ...
        let guest_gdt = state.extended_registers.gdtr.base as u64;

        vmwrite(vmcs::guest::ES_SELECTOR, extended_registers.es);
        vmwrite(vmcs::guest::CS_SELECTOR, extended_registers.cs);
        vmwrite(vmcs::guest::SS_SELECTOR, extended_registers.ss);
        vmwrite(vmcs::guest::DS_SELECTOR, extended_registers.ds);
        vmwrite(vmcs::guest::FS_SELECTOR, extended_registers.fs);
        vmwrite(vmcs::guest::GS_SELECTOR, extended_registers.gs);
        ...
        vmwrite(vmcs::guest::SS_BASE, extended_registers.ss_base);
        vmwrite(vmcs::guest::DS_BASE, extended_registers.ds_base);
        vmwrite(vmcs::guest::FS_BASE, extended_registers.fs_base);
        vmwrite(vmcs::guest::GS_BASE, extended_registers.gs_base);
        vmwrite(vmcs::guest::TR_BASE, extended_registers.tr_base);
        vmwrite(vmcs::guest::LDTR_BASE, extended_registers.ldtr_base);

        vmwrite(vmcs::guest::GDTR_BASE, extended_registers.gdtr.base as u64);
        vmwrite(vmcs::guest::GDTR_LIMIT, extended_registers.gdtr.limit);
        ...
        // See 25.8 Restrictions on VMX operation
        ...
        self.registers.rflags = registers.rflags;
    ...
    fn nps_entry_flags(
        ...
        match entry_type {
            ...
            // W
            NestedPagingStructureEntryType::W => NestedPagingStructureEntryFlags {
                permission: 0b010,
                memory_type: 0,
            },
            // R
            ...
            // RW
            NestedPagingStructureEntryType::Rw => NestedPagingStructureEntryFlags {
                permission: 0b011,
                memory_type: 0,
            },
            // RX
...
fn adjust_cr0_host() {
    cr0_write(adjust_cr0(cr0()));
}

fn adjust_cr4(cr4: Cr4) -> Cr4 {
    let fixed0_cr4 = Cr4::from_bits_truncate(rdmsr(x86::msr::IA32_VMX_CR4_FIXED0) as usize);
    let fixed1_cr4 = Cr4::from_bits_truncate(rdmsr(x86::msr::IA32_VMX_CR4_FIXED1) as usize);
    let new_cr4 = (cr4 & fixed1_cr4) | fixed0_cr4;
    new_cr4
}
